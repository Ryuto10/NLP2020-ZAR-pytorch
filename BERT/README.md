# Tutorial of Pretrained BERT
## Dates
1. 3/26 (Tue) 11:00~ 
2. 3/29 (Fri) 10:30~
3. 4/4 (Thu) 13:00~ (upcoming)

## Instllation
1. Create new environment  `conda create -n pretrained-BERT py=3.6`
    1. (and activate it) `source activate pretrained-BERT`
2. Clone this repo: `git clone git@github.com:cl-tohoku/tutorial-of-pretrained-BERT.git`
3. `cd` to cloned repo `tutorial-of-pretrained-BERT`
4. `pip install --editable .`
5. `cd` to `examples` dir
6. `mkdir result`

You may want to clone the repo to local PC and then sync it to the server (that is how I edit the code).
...
Make sure you set valid `<GPU_ID>` (btw, you should always check http://www.cl.ecei.tohoku.ac.jp/gpus and look for available GPUs)

BERT models are already downloaded to `/work01/share_data`. （気が利くでしょう？？）
Thus, specifying `--cache_dir` prevents downloading 400MB of model parameters.
...
CUDA_VISIBLE_DEVICES=<GPU_ID> python run_classifier.py \
  ...
  --cache_dir /work01/share_data/bert_pretrained_model
```

## Exercise
### Write Code for SST-2
Currently `run_classifier.py` does not support the training with `SST-2` dataset.
Write code on your own so that you can train BERT for `SST-2`.

### Validation for Every Epoch
`run_classifier.py` runs the evaluation code on the validation set **AFTER** the whole training is completed.
Write code so that validation runs every epoch.

### Evaluation on Test Data
`run_classifier.py` does not support evaluation with test data.
Add `do_predict` option to `run_classifier.py` so that the evaluation works with test data.

## Miscellaneous Notes
- You should pay attention to...
  - lowercase the characters or not (depends on which bert model you want to use)
    - not lowercasing the tokens would lead to so many <unk> tokens in the sequence
  - `max_seq_length` argument (I guess this is due to the GPU memory issue. Sequences that are longer than this value will be truncated)
    - Every instance is padded to `max_seq_length` (easier to make mini-batch)
  - `train_batch_size` argument: setting this too large causes `OutOfMemoryError` (remember: BERT itself is already a huge network!)
