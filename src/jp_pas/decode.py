...
def decode_scores(out_dir, data, tag, model, model_id):
    ...
    for xss, yss in tqdm(data, mininterval=5):
        ...
        for pred_no in range(len(yss)):
            ...
            if tag == "train":
                p_id, sent_id, doc_name, golds = yss[pred_no]
                out_dict = {"pred": p_id, "sent": sent_id, "file": doc_name, "scores": predict.tolist(), "golds": golds.tolist()}
            else:
                p_id, sent_id, doc_name = yss[pred_no]
...
def calculate_train_threshold(train_args, model):
    ...
    data_train = NtcBucketIterator(data_train, train_args["batch_size"],
                                   ...
                                   zero_drop=train_args.get("zero_drop"), bert=train_args.get("bert"),
                                   loss_stop=train_args.get("loss_stop"), load_cpu=train_args.get("load_cpu"),
                                   mapping_pseudo_train=train_args.get("mapping_pseudo_train"),
                                   bert_embed_file=train_args.get("train_bert_embed_file"),
                                   pseudo_bert_embed_file=train_args.get("pseudo_bert_embed_file"))
    ...
    model.eval()
    thres, *_ = packed_evaluate_multiclass_without_none(model, data_train, len(data_train), labels, thres_lists)
...
def main():
    ...
    model = PackedE2EStackedBiRNN(hidden_dim=train_args["hidden_dim"],
    ...
    if args.tag == "train":
        logger.info("Use train dataset")
        data_train = load_dataset(train_args["train"], train_args["data_size"])
        if train_args["train_method"] == "concat":
            data_pseudo = load_dataset(train_args["pseudo"], train_args["data_size"]) if train_args["pseudo"] else []
            data_train += data_pseudo
        data = NtcBucketIterator(data_train, train_args["batch_size"],
                                 multi_predicate=train_args["multi_predicate"], full_ys=True,
                                 zero_drop=train_args.get("zero_drop"), bert=train_args.get("bert"),
                                 loss_stop=train_args.get("loss_stop"), load_cpu=train_args.get("load_cpu"),
                                 mapping_pseudo_train=train_args.get("mapping_pseudo_train"),
                                 bert_embed_file=train_args.get("train_bert_embed_file"),
