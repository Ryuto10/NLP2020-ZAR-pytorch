...
from allennlp.training.metrics.srl_eval_scorer import DEFAULT_SRL_EVAL_PATH
from evaluator import SrlEvalScorer
...
class SpanSelectionModel(nn.Module):
    def __init__(self, hidden_dim: int, out_dim: int):
        super(SpanSelectionModel, self).__init__()
...
class SemanticRoleLabelerWithAttention(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 use_multi_predicate: bool = False,
                 initializer: InitializerApplicator = InitializerApplicator(),
                 regularizer: Optional[RegularizerApplicator] = None,
                 label_smoothing: float = None,
                 ignore_span_metric: bool = False,
                 srl_eval_path: str = DEFAULT_SRL_EVAL_PATH) -> None:
        super(SemanticRoleLabelerWithAttention, self).__init__(vocab, regularizer)
        ...
        self.use_multi_predicate = use_multi_predicate
        if use_attention:
            self.attention = MultiheadAttention(hidden_dim, 1, dropout=attention_dropout)
        self.binary_feature_dim = binary_feature_dim * 2 if use_multi_predicate else binary_feature_dim
        # There are exactly 2 binary features for the verb predicate embedding.
        self.binary_feature_embedding = Embedding(2, binary_feature_dim)
        self.multi_verb_binary_feature_embedding = Embedding(2, binary_feature_dim)
        ...
        check_dimensions_match(text_field_embedder.get_output_dim() + self.binary_feature_dim,
    ...
    def forward(self,  # type: ignore
                ...
                multi_verb_indicator: torch.LongTensor,
        ...
        multi_verb_indicator: torch.LongTensor, required.
            他の述語の位置にも1が立っているvector
        ...
        embedded_multi_verb_indicator = self.multi_verb_binary_feature_embedding(multi_verb_indicator.long())
        # Concatenate the verb feature onto the embedded text. This now
        # has shape (batch_size, sequence_length, embedding_dim + binary_feature_dim).

        if self.use_multi_predicate:
            embedded_text_with_verb_indicator = torch.cat([embedded_text_input,
                                                           embedded_verb_indicator,
                                                           embedded_multi_verb_indicator], -1)
        else:
        ...
        if self.use_attention:
            ...
            encoded_text, attn_output_weights = self.attention(encoded_text, encoded_text, encoded_text)
            encoded_text = encoded_text.transpose(0, 1)
